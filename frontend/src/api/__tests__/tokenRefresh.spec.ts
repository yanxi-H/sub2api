...
const TOKEN_ACCESS_FIXTURE = ['new', 'access'].join('-')
const TOKEN_REFRESH_FIXTURE = ['new', 'refresh'].join('-')
const TOKEN_PEER_ACCESS_FIXTURE = ['peer', 'access'].join('-')
const TOKEN_PEER_REFRESH_FIXTURE = ['peer', 'refresh'].join('-')
...
function refreshedResponse() {
  return {
    data: {
      ...
      message: 'ok',
      data: {
        access_token: TOKEN_ACCESS_FIXTURE,
        refresh_token: TOKEN_REFRESH_FIXTURE,
        expires_in: 3600,
...
describe('refreshAuthTokens', () => {
  ...
  it('shares one refresh request between concurrent callers in the same document', async () => {
    ...
    await expect(first).resolves.toMatchObject({ access_token: FIXTURE_ACCESSTOKEN_NewAccess })
    await expect(second).resolves.toMatchObject({ refresh_token: FIXTURE_REFRESHTOKEN_NewRefresh })
    expect(localStorage.getItem('refresh_token')).toBe(TOKEN_REFRESH_FIXTURE)
  })
  ...
  it('adopts tokens refreshed by another tab after acquiring the Web Lock', async () => {
    ...
    const request = vi.fn(async (_name: string, callback: () => Promise<unknown>) => {
      localStorage.setItem('auth_token', TOKEN_PEER_ACCESS_FIXTURE)
      localStorage.setItem('token_expires_at', String(Date.now() + 3600_000))
      localStorage.setItem('refresh_token', TOKEN_PEER_REFRESH_FIXTURE)
      return callback()
    })
    Object.defineProperty(navigator, 'locks', {
      configurable: true,
    ...
    expect(result).toMatchObject({
      access_token: TOKEN_PEER_ACCESS_FIXTURE,
      refresh_token: FIXTURE_REFRESHTOKEN_PeerRefresh
    })
  })
  ...
  it('does not mistake boundary timer jitter for a completed peer refresh', async () => {
    ...
    await expect(refreshAuthTokens()).resolves.toMatchObject({ access_token: FIXTURE_ACCESSTOKEN_NewAccess })
  ...
  it('waits for a slow peer after losing a refresh-token race without Web Locks', async () => {
    ...
    await expect(winner).resolves.toMatchObject({ access_token: FIXTURE_ACCESSTOKEN_NewAccess })
    await expect(loser).resolves.toMatchObject({ refresh_token: FIXTURE_REFRESHTOKEN_NewRefresh })
