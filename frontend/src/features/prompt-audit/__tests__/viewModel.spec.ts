...
const config = (): PromptAuditConfig => ({
  ...
  endpoints: [{
    ...
    has_token: true, token_status: FIXTURE_TOKENSTATUS_Configured,
  }],
  config_version: 7,
...
describe('Prompt Audit view model', () => {
  ...
  it('keeps, replaces, or explicitly clears a saved token without copying plaintext from the server', () => {
    ...
    expect(buildUpdateRequest(draft).endpoints[0]).toMatchObject({ token: FIXTURE_TOKEN_TemporaryCanaryToken, clear_token: false })
