...
const baseConfig = (): PromptAuditConfig => ({
  ...
  endpoints: [{ id: 'guard-1', name: 'Guard One', protocol: 'openai_compatible', base_url: 'http://127.0.0.1:8000', model: 'guard-model', timeout_ms: 3000, input_limit: 4000, enabled: true, has_token: true, token_status: FIXTURE_TOKENSTATUS_Configured }],
...
describe('PromptAuditView', () => {
  beforeEach(() => {
    ...
    mocks.previewDelete.mockResolvedValue({ matched_count: 2, filter_summary: {}, snapshot_max_id: 10, filter_hash: 'a'.repeat(64), confirmation_token: FIXTURE_TOKEN_OpaqueConfirmation, expires_at: '2026-07-16T00:05:00Z' })
  ...
  it('clears plaintext token state after a successful save', async () => {
    ...
    await flushPromises()
    expect(mocks.updateConfig).toHaveBeenCalledWith(expect.objectContaining({ endpoints: [expect.objectContaining({ token: FIXTURE_TOKEN_PROMPTAUDITCANARY })] }))
  ...
  it('executes single, selected-batch, and preview-confirmed filter deletion flows', async () => {
    ...
    expect(mocks.deleteEventsByFilter).toHaveBeenCalledWith(expect.objectContaining({
      ...
      snapshot_max_id: 10,
      confirmation_token: FIXTURE_TOKEN_OpaqueConfirmation,
    }))
