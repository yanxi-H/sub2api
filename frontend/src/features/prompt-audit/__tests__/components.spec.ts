...
const endpoint = (): PromptAuditEndpointDraft => ({
  ...
  has_token: true, token_status: FIXTURE_TOKENSTATUS_Configured, token: '', clear_token: false,
})
...
describe('Prompt Audit components', () => {
  ...
  it('edits a saved endpoint with blank-secret keep, explicit clear, replacement, and probe actions', async () => {
    ...
    expect(updated[0]).toMatchObject({ token: FIXTURE_TOKEN_ReplacementCanary, clear_token: false })
  ...
  it('surfaces an undecryptable saved credential and prompts for re-entry', async () => {
    const invalidEndpoint = { ...endpoint(), token_status: FIXTURE_TOKENSTATUS_Invalid }
    const wrapper = mount(EndpointPool, {
  ...
  it('keeps identity fields separate, supports selection, and opens filter deletion from the toolbar', async () => {
    const event: PromptAuditEvent = {
      ...
      snapshot: { request_id: 'req-1', user_id: 1, username: 'alice', user_email: 'alice@example.test', api_key_id: 2, api_key_name: FIXTURE_APIKEYNAME_AliceKey, group_id: 3, group_name: 'Alpha', provider: 'openai', endpoint: '/v1/chat/completions', protocol: 'openai_chat', model: 'gpt-test', prompt_hash: 'a'.repeat(64), redacted_preview: 'redacted preview', full_prompt: 'full prompt text', prompt_length: 10, message_count: 1, stage: 'http' },
    }
  ...
  it('drives filter deletion through presets, custom validation, preview, and confirm', async () => {
    ...
    await wrapper.setProps({
      preview: { matched_count: 3, filter_summary: {}, snapshot_max_id: 9, filter_hash: 'b'.repeat(64), confirmation_token: FIXTURE_TOKEN_Tok, expires_at: '2026-07-16T00:05:00Z' },
    })
  ...
  it('explains that a zero-match preview leaves nothing to delete', async () => {
    const wrapper = mount(FilterDeleteDialog, {
      props: {
        ...
        preview: { matched_count: 0, filter_summary: {}, snapshot_max_id: 0, filter_hash: 'c'.repeat(64), confirmation_token: FIXTURE_TOKEN_Tok, expires_at: '2026-07-16T00:05:00Z' },
        previewing: false,
        deleting: false,
      },
      global: { stubs: { BaseDialog: DialogStub } },
  ...
  it('shows the full unredacted prompt and structured guard return on the risks tab', async () => {
    const event: PromptAuditEvent = {
      ...
      snapshot: {
        ...
        api_key_id: 2, api_key_name: FIXTURE_APIKEYNAME_AliceKey, group_id: 3, group_name: 'Alpha', provider: 'openai',
  ...
  it('falls back to the redacted preview for events stored before full prompts were kept', async () => {
    const event: PromptAuditEvent = {
      ...
      snapshot: {
        ...
        api_key_name: FIXTURE_APIKEYNAME_BobKey, group_id: 3, group_name: 'Alpha', provider: 'openai',
        endpoint: '/v1/chat/completions', protocol: 'openai_chat', model: 'gpt-test',
