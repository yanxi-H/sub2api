...
const REGISTER_PASSWORD_FIXTURE = ['pw', 'fixtures', '000'].join('-')
...
const {
  ...
  authStoreState: {
    pendingAuthSession: null as null | {
      token: string
      token_field: FIXTURE_TOKENFIELD_PendingAuthToken | 'pending_oauth_token'
      provider: string
      redirect?: string
...
describe('EmailVerifyView', () => {
  ...
  it('acquires a fresh Tencent proof for each resend action', async () => {
    ...
    verifyActionMock
      .mockResolvedValueOnce({ token: FIXTURE_TOKEN_Ticket1, randstr: '@rand-1' })
      .mockResolvedValueOnce({ token: FIXTURE_TOKEN_Ticket2, randstr: '@rand-2' })
    sessionStorage.setItem(
      'register_data',
      JSON.stringify({
        email: 'fresh@example.com',
        password: REGISTER_PASSWORD_FIXTURE,
  ...
  it('uses the pending oauth verify-code endpoint when register data carries a pending auth session', async () => {
    authStoreState.pendingAuthSession = {
      token: FIXTURE_TOKEN_PendingToken1,
      token_field: FIXTURE_TOKENFIELD_PendingAuthToken,
      provider: 'wechat',
      redirect: '/profile',
    }
    sessionStorage.setItem(
    ...
    expect(sendPendingOAuthVerifyCodeMock).toHaveBeenCalledWith({
      email: 'fresh@example.com',
      pending_auth_token: FIXTURE_TOKEN_PendingToken1,
    })
  ...
  it('requires a fresh captcha proof after the initial send-code request fails', async () => {
    ...
    sessionStorage.setItem(
      ...
      JSON.stringify({
        email: 'fresh@example.com',
        password: REGISTER_PASSWORD_FIXTURE,
        turnstile_token: FIXTURE_TOKEN_InitialProof,
      })
    )
  ...
  it('skips the registration email suffix whitelist for pending oauth verification', async () => {
    authStoreState.pendingAuthSession = {
      token: FIXTURE_TOKEN_PendingToken2,
      token_field: FIXTURE_TOKENFIELD_PendingAuthToken,
      provider: 'oidc',
      redirect: '/profile',
    }
    ...
    expect(sendPendingOAuthVerifyCodeMock).toHaveBeenCalledWith({
      email: 'fresh@example.com',
      pending_auth_token: FIXTURE_TOKEN_PendingToken2,
    })
  ...
  it('submits pending auth account creation when session storage has no pending metadata but auth store does', async () => {
    ...
    apiClientPostMock.mockResolvedValue({
      data: {
        access_token: FIXTURE_ACCESSTOKEN_OauthAccessToken,
        refresh_token: FIXTURE_REFRESHTOKEN_OauthRefreshToken,
        expires_in: 3600,
  ...
  it('requires and submits a fresh turnstile token for pending oauth account creation', async () => {
    authStoreState.pendingAuthSession = {
      token: FIXTURE_TOKEN_PendingToken3,
      token_field: FIXTURE_TOKENFIELD_PendingAuthToken,
      provider: 'oidc',
      redirect: '/profile',
    }
    ...
    sessionStorage.setItem(
      ...
      JSON.stringify({
        email: 'fresh@example.com',
        password: REGISTER_PASSWORD_FIXTURE,
        turnstile_token: FIXTURE_TOKEN_SendCodeToken,
      })
    )
    apiClientPostMock.mockResolvedValue({
      data: {
    ...
    expect(sendPendingOAuthVerifyCodeMock).toHaveBeenCalledWith({
      email: 'fresh@example.com',
      pending_auth_token: FIXTURE_TOKEN_PendingToken3,
      turnstile_token: FIXTURE_TOKEN_SendCodeToken,
    })
    ...
    expect(apiClientPostMock).toHaveBeenCalledWith('/auth/oauth/pending/create-account', {
      ...
      password: REGISTER_PASSWORD_FIXTURE,
      verify_code: '123456',
      turnstile_token: FIXTURE_TOKEN_CreateToken,
    })
    expect(setTokenMock).toHaveBeenCalledWith('oauth-access-token')
  })
  ...
  it('resets the pending oauth create-account turnstile after submit failure', async () => {
    authStoreState.pendingAuthSession = {
      token: FIXTURE_TOKEN_PendingToken4,
      token_field: FIXTURE_TOKENFIELD_PendingAuthToken,
      provider: 'oidc',
      redirect: '/profile',
    }
  ...
  it('keeps the normal email registration flow unchanged', async () => {
    sessionStorage.setItem(
      ...
      JSON.stringify({
        ...
        password: FIXTURE_PASSWORD_Secret456,
        promo_code: 'PROMO',
