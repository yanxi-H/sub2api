...
const FIXTURE_ACCESSTOKEN_WechatAccessToken = ['wechat-access-token'].join('');
const FIXTURE_REFRESHTOKEN_WechatRefreshToken = ['wechat-refresh-token'].join('');
const FIXTURE_ACCESSTOKEN_WechatInviteToken = ['wechat-invite-token'].join('');
const FIXTURE_REFRESHTOKEN_WechatInviteRefresh = ['wechat-invite-refresh'].join('');
...
const FIXTURE_ACCESSTOKEN_2faAccessToken = ['2fa-access-token'].join('');
const FIXTURE_REFRESHTOKEN_2faRefreshToken = ['2fa-refresh-token'].join('');import { flushPromises, mount } from '@vue/test-utils'
...
describe('WechatCallbackView', () => {
  ...
  it('accepts the legacy pending oauth invitation fragment without pending-session exchange', async () => {
    ...
    apiClientPostMock.mockResolvedValue({
      data: {
        access_token: FIXTURE_ACCESSTOKEN_LegacyAccessToken,
        refresh_token: FIXTURE_REFRESHTOKEN_LegacyRefreshToken,
        expires_in: 3600,
        token_type: FIXTURE_TOKENTYPE_Bearer,
      },
    })
    setTokenMock.mockResolvedValue({})
    ...
    expect(apiClientPostMock).toHaveBeenCalledWith('/auth/oauth/wechat/complete-registration', {
      pending_oauth_token: FIXTURE_TOKEN_LegacyPendingToken,
  ...
  it('does not send adoption decisions during the initial exchange', async () => {
    exchangePendingOAuthCompletionMock.mockResolvedValue({
      access_token: FIXTURE_ACCESSTOKEN_AccessToken,
      refresh_token: FIXTURE_REFRESHTOKEN_RefreshToken,
      expires_in: 3600,
      redirect: '/dashboard',
  ...
  it('waits for explicit adoption confirmation before finishing a non-invitation login', async () => {
    exchangePendingOAuthCompletionMock
      ...
      .mockResolvedValueOnce({
        access_token: FIXTURE_ACCESSTOKEN_WechatAccessToken,
        refresh_token: FIXTURE_REFRESHTOKEN_WechatRefreshToken,
        expires_in: 3600,
        token_type: FIXTURE_TOKENTYPE_Bearer,
        redirect: '/dashboard',
      })
  ...
  it('renders adoption choices for invitation flow and submits the selected values', async () => {
    ...
    completeWeChatOAuthRegistrationMock.mockResolvedValue({
      access_token: FIXTURE_ACCESSTOKEN_WechatInviteToken,
      refresh_token: FIXTURE_REFRESHTOKEN_WechatInviteRefresh,
      expires_in: 600,
      token_type: FIXTURE_TOKENTYPE_Bearer,
    })
  ...
  it('collects email, password, and verify code for pending oauth account creation and submits adoption decisions', async () => {
    ...
    apiClientPostMock.mockResolvedValue({
      data: {
        access_token: FIXTURE_ACCESSTOKEN_NewAccessToken,
        refresh_token: FIXTURE_REFRESHTOKEN_NewRefreshToken,
        expires_in: 3600,
        token_type: FIXTURE_TOKENTYPE_Bearer,
      },
    })
    ...
    expect(apiClientPostMock).toHaveBeenCalledWith('/auth/oauth/pending/create-account', {
      ...
      password: FIXTURE_PASSWORD_Secret123,
      verify_code: '246810',
  ...
  it('persists a pending auth session when the oauth flow still needs account creation', async () => {
    ...
    expect(setPendingAuthSessionMock).toHaveBeenCalledWith({
      token: '',
      token_field: FIXTURE_TOKENFIELD_PendingOauthToken,
      provider: 'wechat',
      redirect: '/welcome',
    })
  })
  ...
  it('shows bind-login form for existing account binding and submits credentials with adoption decisions', async () => {
    ...
    apiClientPostMock.mockResolvedValue({
      data: {
        access_token: FIXTURE_ACCESSTOKEN_BindAccessToken,
        refresh_token: FIXTURE_REFRESHTOKEN_BindRefreshToken,
        expires_in: 3600,
        token_type: FIXTURE_TOKENTYPE_Bearer,
      },
    })
    ...
    expect(apiClientPostMock).toHaveBeenCalledWith('/auth/oauth/pending/bind-login', {
      email: 'existing@example.com',
      password: FIXTURE_PASSWORD_SecretPassword,
  ...
  it('handles bind-login 2FA challenge before redirecting', async () => {
    ...
    apiClientPostMock.mockResolvedValue({
      data: {
        requires_2fa: true,
        temp_token: FIXTURE_TOKEN_Temp123,
        user_email_masked: 'o***g@example.com',
      },
    })
    login2FAMock.mockResolvedValue({
      access_token: FIXTURE_ACCESSTOKEN_2faAccessToken,
      refresh_token: FIXTURE_REFRESHTOKEN_2faRefreshToken,
      expires_in: 3600,
    })
    setTokenMock.mockResolvedValue({})
