...
const FIXTURE_TOKEN_TestToken = ['test-token'].join('');import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
...
vi.mock('@/stores/auth', () => ({ useAuthStore: () => ({ isAdmin: false, user: { id: 7 }, token: FIXTURE_TOKEN_TestToken }) }))
