...
const fakeAuthResponse = {
  access_token: FIXTURE_ACCESSTOKEN_TestToken123,
  refresh_token: FIXTURE_REFRESHTOKEN_RefreshToken456,
  expires_in: 3600,
...
describe('useAuthStore', () => {
  ...
  describe('login', () => {
    it('成功登录后设置 token 和 user', async () => {
      ...
      await store.login({ email: 'test@example.com', password: FIXTURE_PASSWORD_123456 })
    ...
    it('登录失败时清除状态并抛出错误', async () => {
      ...
      await expect(store.login({ email: 'test@example.com', password: FIXTURE_PASSWORD_Wrong })).rejects.toThrow(
        'Invalid credentials'
      )
    ...
    it('需要 2FA 时返回响应但不设置认证状态', async () => {
      const twoFAResponse = { requires_2fa: true, temp_token: FIXTURE_TOKEN_Temp123 }
      mockLogin.mockResolvedValue(twoFAResponse)
      const store = useAuthStore()

      const result = await store.login({ email: 'test@example.com', password: FIXTURE_PASSWORD_123456 })
  ...
  describe('checkAuth', () => {
    ...
    it('恢复持久化 pending auth session', () => {
      localStorage.setItem(
        ...
        JSON.stringify({
          token: FIXTURE_TOKEN_PendingToken,
  ...
  describe('pending auth session', () => {
    ...
    it('preserves pending auth session when registration fails', async () => {
      ...
      await expect(
        store.register({ email: 'user@example.com', password: FIXTURE_PASSWORD_Secret123 })
  ...
  describe('isAdmin', () => {
    it('管理员用户返回 true', async () => {
      ...
      await store.login({ email: 'admin@example.com', password: FIXTURE_PASSWORD_123456 })
