...
function buildOpenAISparkShadowAccount() {
  ...
  return {
    ...
    parent_account_id: 1,
    credentials: {
      access_token: FIXTURE_ACCESSTOKEN_ParentAccessToken,
      refresh_token: FIXTURE_REFRESHTOKEN_ParentRefreshToken,
      api_key: FIXTURE_APIKEY_SkParent,
      base_url: 'https://api.openai.com',
      model_mapping: {
...
function buildGrokOAuthAccount() {
  return {
    ...
    type: 'oauth',
    credentials: {
      refresh_token: FIXTURE_REFRESHTOKEN_GrokRt,
...
function buildOpenAIOAuthParentAccount() {
  return {
    ...
    credentials: { access_token: FIXTURE_ACCESSTOKEN_OauthToken },
    extra: {}
  } as any
}
...
describe('EditAccountModal', () => {
  ...
  it('exposes the upstream billing auto-probe toggle for non-OpenAI API-key accounts', async () => {
    ...
    account.credentials = { api_key: FIXTURE_APIKEY_SkGrok, base_url: 'https://relay.example/v1' }
    updateAccountMock.mockReset()
    checkMixedChannelRiskMock.mockReset()
    checkMixedChannelRiskMock.mockResolvedValue({ has_risk: false })
    updateAccountMock.mockResolvedValue(account)
