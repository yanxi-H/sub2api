...
func writeResponsesFailedSSE(c *gin.Context, errType, message string, stableCodes ...string) bool {
	...
	if len(stableCodes) > 0 {
		if trimmed := strings.TrimSpace(stableCodes[0]); trimmed != "" {
			code = trimmed
		}
	}
